...
<div class="pt-20 pb-100 md:pb-55 -mx-4 <?php echo $classes[0]; ?>">
	<div class="container <?php echo $classes[1]; ?>">
		<h2>
			...
			<span data-aos="fade-in" data-aos-delay="50"><?php echo str_replace("\n", '<br />',  $attributes['title']); ?></span>
		</h2>
		...
		<ul class="md:grid my-0 -mx-1 lg:-mx-3 p-0 list-none justify-center <?php echo $classes[2]; ?>">
			...
			<?php $count = 0; while ( $loop->have_posts() ) : $loop->the_post(); ?>
				<li data-aos="eco-scroll" data-aos-delay="<?php echo $count; ?>" class="grow m-0 p-1 lg:p-3">
					<a class="group flex flex-wrap content-between h-full not-prose p-5 pt-10 text-tertiary rounded-lg bg-foreground hover:-translate-y-1 hover:bg-primary transition" href="<?php echo get_permalink(); ?>" title="<?php echo get_the_title(); ?>" rel="bookmark">
						<span class="block font-semibold text-2xl lg:text-4xl transition"><?php the_title(); ?></span>
						<span class="block w-full mt-20 text-sm">
							<span class="block mr-10 group-hover:-translate-y-2 transition"><?php echo get_the_excerpt(); ?></span>
